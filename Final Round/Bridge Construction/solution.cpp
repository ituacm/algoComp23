// In this solution, the bridge is constructed to only sea cells.
   ...
   public:
    ...
    void changeFourAdjacentCell(int i, int j, int remaining, set<pair<int, int>>& visited, vector<vector<int>>& grid, unordered_map<int, int>& sizeOfIslands) {
        ...
        for (int k = 0; k < dir_x.size(); k++) {
            ...
            if (next_i >= 0 && next_i < grid.size() && next_j >= 0 && next_j < grid.size() && grid[next_i][next_j] == 0 && visited.find({next_i, next_j}) == visited.end()) {
